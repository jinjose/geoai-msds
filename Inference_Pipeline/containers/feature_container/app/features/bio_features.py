...
def weather_features(wx_year, month, day, hist_mean_temp=None):
    ...
    # --- DEDUP: protect against duplicate daily rows per county/date ---
    # If the raw data accidentally has duplicate rows for the same day,
    # sums and counts (rain_sum_mm, heat_days_gt32, etc.) would be inflated.
    agg_cols = [c for c in ["temperature", "rain_mm", "et_mm", "water_balance_mm", "vpd_kpa"] if c in wd.columns]
    if agg_cols:
        wd = wd.groupby("date", as_index=False)[agg_cols].mean()
    else:
        wd = wd.drop_duplicates(subset=["date"])
