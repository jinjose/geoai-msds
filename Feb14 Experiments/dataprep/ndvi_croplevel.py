...
# -----------------------------------------------------------
START_YEAR = 2013
END_YEAR = 2024  # CDL lags ~1 year

# -----------------------------------------------------------
# 3. Iowa counties
# -----------------------------------------------------------
counties = ee.FeatureCollection("TIGER/2018/Counties")
iowa_counties = counties.filter(ee.Filter.eq("STATEFP", "19"))  # Iowa = 19

# -----------------------------------------------------------
# 4. Output directory
# -----------------------------------------------------------
OUT_DIR = Path("<YOUR__DIR>")
...
def extract_multiple_years(start_year, end_year):
    ...
    for year in range(start_year, end_year + 1):
        ...
        df_year.to_csv(
            OUT_DIR / f"corn_ndvi_iowa_county_{year}.csv",
            index=False
        )
    ...
    final_df.to_csv(
        OUT_DIR / "corn_ndvi_iowa_county_all_years.csv",
        index=False
    )

    print("\n✔ County-level CORN NDVI extraction complete")
    print(f"Saved to: {OUT_DIR / 'corn_ndvi_iowa_county_all_years_new.csv'}")
